    ...
    fn traduire(&self, arguments: Vec<String>) -> Vec<Valeurs> {
        ...
        for argument in arguments {
            if let Some(p) = argument.find(':') {
                match &argument[..p] {
                    "booléen" => match &argument[p + 1..] {
                        "vrai" | "1" => r.push(Valeurs::Booleen(true)),
                        "faux" | "0" => r.push(Valeurs::Booleen(false)),
                        _ => panic!("valeur booléenne incorrecte"),
                    },
                    "relatif" => {
                        if let Ok(n) = &argument[p + 1..].parse::<i32>() {
                            r.push(Valeurs::Relatif(*n));
                        } else {
                    ...
                    "flottant" => {
                        if let Ok(n) = &argument[p + 1..].parse::<f32>() {
                            r.push(Valeurs::Flottant(*n));
                        } else {
                            panic!("valeur flottante incorrecte")
                        }
                    }
                    "texte" => {
                        r.push(Valeurs::Texte(argument[p + 1..].to_string()));
                    }
                    _ => panic!("type inconnu"),
                };
            } else {
                r.push(Valeurs::Texte(argument.to_string()));
            }
        }
        r
    }
            ...
            Commandes::Groupe(_, groupe) => {
                while self.liste.len() > 0 {
                    ...
                    match &commande.fct[..] {
                        ...
                        "converger" => return,
                        _ => groupe.push(Commandes::Commande(
                            match &commande.etat[..] {
                                "+" => CommandesFiltre::Positif,
                                "-" => CommandesFiltre::Negatif,
                                "ø" => CommandesFiltre::Inconditionnel,
...
fn filtrer_commande(contexte: &mut ContexteExecution, commande: &mut Commandes) {
    match commande {
        Commandes::Commande(filtre, appel, args) => match filtre {
            CommandesFiltre::Positif if contexte.etat() => {
                executer_commande(contexte, &appel[..], &args)
            }
            CommandesFiltre::Negatif if !contexte.etat() => {
                executer_commande(contexte, &appel[..], &args)
            }
...
pub fn tester(contexte_client: TcpStream, script: &str, valeur_visee: &mut Valeurs) -> bool {
    ...
    // (2)
    let mut expressions = Expressions { liste: items };
    let mut racine = Commandes::Groupe(CommandesFiltre::Positif, Vec::new());
    expressions.parcourir(&mut racine);
