...
///
fn resoudre_incrementer(
    contexte: &mut Contexte,
    mut arguments: grammaire::ArgumentsLocaux,
) -> Retour {
    ...
    let r = match grammaire::chemin_extraire(&arg_chemin) {
        Ok(chemin) => canal.resoudre(&chemin, |valeur| match valeur {
            Valeurs::Relatif(v) => match i.parse::<i32>() {
                Ok(n) => {
                    *v += n;
            ...
            Valeurs::Flottant(v) => match i.parse::<f32>() {
                Ok(n) => {
                    *v += n;
                    Retour::creer_str(true, "incrémentation effectuée")
                }
            ...
            _ => Retour::creer_str(
                false,
                "incrémentation impossible sur une valeur non-numérique",
            ),
        }),
