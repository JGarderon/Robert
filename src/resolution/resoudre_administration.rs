    ...
    match arguments.tous() {
        Ok(v) => {
            if v.len() == 2 {
                ...
                Retour::creer_str(false, "deux arguments obligatoires : 'pseudo' et 'passe'")
            }
        }
        Err(_) => return Retour::creer_str(false, "arguments invalides"),
    }
}
...
fn resoudre_anonymiser(contexte: &mut Contexte, _: ArgumentsLocaux) -> Retour {
    contexte.message(if contexte.profil.est_authentifie() {
        "vous étiez authentifié"
    } else {
        "vous n'étiez pas authentifié"
    });
    contexte.profil.anonymiser();
    Retour::creer_str(true, "anonymisation réussie")
}
...
fn resoudre_profiler(contexte: &mut Contexte, _: ArgumentsLocaux) -> Retour {
    ...
    Retour::creer_str(true, "profilage réussi")
}
...
fn resoudre_serialiser(contexte: &mut Contexte, mut arguments: ArgumentsLocaux) -> Retour {
    ...
    if fid.len() > 32 {
        return Retour::creer_str(
            false,
            "l'identifiant de dump doit faire 32 caractères maximum",
        );
    }
    ...
    if let Some(n) = canal.liste.serialiser(&mut s) {
...
fn resoudre_resumer(contexte: &mut Contexte, mut arguments: ArgumentsLocaux) -> Retour {
    ...
    Retour::creer(
        true,
        format!("nbre total : {} (taille totale : {})", nbre, total),
    )
}
...
pub fn resoudre(appel: &str) -> Result<Resolveur, Retour> {
    match appel {
        ...
        _ => Err(Retour::creer_str(
            false,
            "module 'administration' : fonction inconnue",
        )),
    }
}
