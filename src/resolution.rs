...
fn resoudre_definir(contexte: &mut Contexte, mut arguments: ArgumentsLocaux) -> Retour {
    ...
    let valeur = if let Some(v) = arguments.extraire() {
        ...
        return Retour::creer_str(
            false,
            "aucune valeur fournie ou séparateur clé/valeur non-respecté",
        );
    };
    ...
    let r = match grammaire::chemin_extraire(&arg_chemin) {
        Ok(chemin) => {
            if chemin.len() == 1 {
                canal
                    .liste
                    .creer_valeur(chemin[0].to_string(), valeur, valeur_type)
            } else {
                let cle = chemin[chemin.len() - 1].to_string();
                canal.resoudre(&chemin[..chemin.len() - 1], move |parent| {
                    parent.creer_valeur(cle, valeur, valeur_type)
                })
            }
        }
...
fn resoudre_supprimer(contexte: &mut Contexte, mut arguments: ArgumentsLocaux) -> Retour {
    ...
    let mut canal = acces_canal!(contexte);
    let r = match grammaire::chemin_extraire(&arg_chemin) {
        Ok(chemin) => canal.resoudre(&chemin[..chemin.len() - 1], |parent| match parent {
            Valeurs::Objet(h) => {
                if let Some(_) = h.remove(&chemin[chemin.len() - 1].to_string()) {
...
fn resoudre_tester(contexte: &mut Contexte, mut arguments: ArgumentsLocaux) -> Retour {
    ...
    let mut canal = acces_canal!(contexte);
    match grammaire::chemin_extraire(&arg_chemin) {
        Ok(chemin) => canal.resoudre(&chemin, |_| Retour::creer_str(true, "ce chemin existe")),
...
fn resoudre_alterer(contexte: &mut Contexte, mut arguments: ArgumentsLocaux) -> Retour {
    ...
    let valeur_type = if let Some(vt) = arguments.extraire() {
        ...
        return Retour::creer_str(
            false,
            "vous devez spécifier un type correct pour l'altération",
        );
    };
    let mut canal = acces_canal!(contexte);
    let r = match grammaire::chemin_extraire(&arg_chemin) {
        Ok(chemin) => canal.resoudre(&chemin, |valeur| {
    ...
    if r.etat {
        canal.notifier(
            &contexte.profil,
            format!("(général) altération : {} ({})", arg_chemin, valeur_type),
        );
    }
    r
}
